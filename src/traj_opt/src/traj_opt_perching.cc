...
static Eigen::Vector3d car_p_, car_v_;//栖息平台的位置和速度
static Eigen::Vector3d tail_q_v_;//栖息平台的姿态zd
static Eigen::Vector3d g_(0, 0, -9.8);
static Eigen::Vector3d land_v_;
static Eigen::Vector3d xb_;
static Eigen::Vector3d v_t_x_, v_t_y_;//zb的正交向量
static Trajectory init_traj_;
static double init_tail_f_;//终端推力初始值
static Eigen::Vector2d init_vt_;//终端切向速度初始值
...
static void forwardTailV(const Eigen::Ref<const Eigen::Vector2d>& xy,
  ...
  tailV = land_v_ + xy.x() * v_t_x_ + xy.y() * v_t_y_;//vn在landv中
}
...
bool TrajOpt::generate_traj(const Eigen::MatrixXd& iniState,
  ...
  visualization_msgs::Marker midpt;            
  midpt.type = visualization_msgs::Marker::SPHERE;
  midpt.action = visualization_msgs::Marker::ADD;
  midpt.header.frame_id = "world";
  midpt.scale.x = 0.1;
  midpt.scale.y = 0.1;
  midpt.scale.z = 0.1;
  midpt.color.a = 1.0;
  ...
  if (opt_once) {//已有初始解
    ...
    {
      mid_pos.head(2) = car_p_.head(2)-len_temp/xb_.norm()*xb_.head(2);
      mid_pos.z() = car_p_.z() + len_temp/xb_.norm()*xb_.z();
    }
  ...
  std::cout << "maxVel: " << traj.getMaxVelRate() << std::endl;
  std::cout << "maxAcc: " << traj.getMaxAccRate() << std::endl;
...
void TrajOpt::addTimeIntPenalty(double& cost) {
  ...
  double cost_floor = 0; 
  double cost_v = 0;
  double cost_thrust = 0;
  double cost_omega = 0;
  double cost_perchCollision = 0;
  ...
  for (int j = 0; j < innerLoop; ++j) {
    ...
    for (int i = 0; i < N_; ++i) {
      ...
      if (grad_cost_v(vel, grad_tmp, cost_tmp)) {
        grad_v += grad_tmp;
        cost_inner += cost_tmp;
        cost_v += omg * step * cost_tmp;
      }

      if (grad_cost_thrust(acc, grad_tmp, cost_tmp)) {
        grad_a += grad_tmp;
        cost_inner += cost_tmp;
        cost_thrust += omg * step * cost_tmp;
      }
      ...
      if (grad_cost_perching_collision(pos, acc, car_p,
        ...
        grad_a += grad_tmp2;
        cost_inner += cost_tmp;
        cost_perchCollision += omg * step * cost_tmp;
      }
...
//              -ab/(1+c), 1-b^2/(1+c), -b]
bool TrajOpt::grad_cost_perching_collision(const Eigen::Vector3d& pos,//pos是当前无人机位置
                                           const Eigen::Vector3d& acc,
                                           const Eigen::Vector3d& car_p,//当前栖息平面位置
                                           Eigen::Vector3d& gradp,
                                           Eigen::Vector3d& grada,
  ...
  Eigen::Vector3d grad_carp_dist = -gradp_dist;//carp与时间相关
